...
- Lesson 2 — Marketing: `1g3zNSBKEZzV6jymJ24lYvFHdit3U7Zgh` → [`src/app/pages/marketing/vocab/`](../src/app/pages/marketing/vocab/)
- Lesson 3 — Warranties: `1tio1N9MXgNMcsQLrnlSmAHjJkhDbW3do` → [`src/app/pages/warranty/vocab/`](../src/app/pages/warranty/vocab/) *(reading PDF — vocab trích từ Part 5/6/7 content; không có section "Words To Learn" riêng)*
- Lesson 4 — Business Planning: `1ud1mVIpOJo5DAZPXHBWIjyjVeZNHD416` → [`src/app/pages/business-planning/vocab/`](../src/app/pages/business-planning/vocab/)
...
| **AUTO_PHRASES** (AI-generated) | [`src/app/shared/dict/auto-phrases.json`](../src/app/shared/dict/auto-phrases.json) | `npm run gen-dict` (LLM) | Cụm 2-3 từ tự động dịch (vd: "new store", "satisfied customers", "look forward to") |
| **AUTO_DICT** (AI-generated) | [`src/app/shared/dict/auto-dict.json`](../src/app/shared/dict/auto-dict.json) | `npm run gen-dict` (LLM) | Tự động dịch mọi từ đơn trong content data |

**Runtime tokenizer — longest-match:**
1. Manual markup `[phrase]{vi}` luôn ưu tiên cao nhất.
2. Trong phần text còn lại, tại mỗi vị trí từ:
   - Thử match cụm 3-từ trong AUTO_PHRASES → nếu có, mark cả cụm
   - Nếu không, thử cụm 2-từ
   - Nếu không, lookup từ đơn: COMMON_DICT → AUTO_DICT (lemma fallback)
3. Cụm chỉ match khi separator giữa các từ là **khoảng trắng thuần** (không vượt qua dấu phẩy/chấm).
...
1. **Setup 1 lần** — set API key của 1 trong các provider (script tự detect):
   ...
   **Cách khuyên dùng — file `.env` ở project root** (script tự load, gitignored):
   ```
   GEMINI_API_KEY=AIza...
   ```
   Tham khảo [`.env.example`](../.env.example). File `.env` tuyệt đối không commit.

   **Cách thay thế — env var session:**
   ```powershell
   $env:GEMINI_API_KEY = "..."
   ```
